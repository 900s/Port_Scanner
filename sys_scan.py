    ...
    def main(self):
        ...
        if len(sys.argv) == 2:
            ...
            if sys.argv[1] in ["-h", "--help"]:
                print("python sys_scan.py <ip> ------ 扫描默认端口")
                print("python sys_scan.py <ip> <port_range> ------ 自定义端口范围进行扫描 (例如扫描81至85号端口: python sys_scan.py 127.0.0.1 81-85)")
                print("python sys_scan.py <ip> <port_list> ------ 选择端口进行扫描 (例如扫描81和83端口: python sys_scan.py 127.0.0.1 81,83)")
            ...
            # 扫描默认端口
                ...
                print("非法输入, 请使用帮助文档检查! (例: python sys_scan.py -h 或 python sys_scan.py --help)")
        elif len(sys.argv) == 3:
            # 选择端口进行扫描
            if "," in sys.argv[2]:
